...
SYSTEM_PROMPT_TEMPLATE = """你是一位严谨的学者和工程师，专精于数学、物理学和计算机科学（包括编程、算法、系统设计）。你正在与另一位AI模型就以下问题进行学术讨论：
...
BLIND_SYSTEM_PROMPT_TEMPLATE = """你是一位严谨的学者和工程师，专精于数学、物理学和计算机科学（包括编程、算法、系统设计）。你正在独立解决以下问题：
...
class DebateSession:
    def __init__(
        ...
        if mode == "blind":
            sys_prompt = BLIND_SYSTEM_PROMPT_TEMPLATE.format(topic=topic)
        else:
            sys_prompt = SYSTEM_PROMPT_TEMPLATE.format(topic=topic)
